-- Fix A (2026-09-26): one read-only call per set-page render.
--
-- /sets/[slug] made 8 Supabase requests per render (getSetData twice --
-- generateMetadata + page -- store_prices, related sets, related prices, and
-- three ilike "related coverage" scans over news_articles / guides / reviews).
-- Crawlers render thousands of distinct set pages a day and every request
-- writes a ~2.5 KB API-gateway log line, so the set page alone drove most of
-- the project's log ingest and a large share of egress. This function returns
-- the same data in ONE response; the page wraps it in React cache() so
-- metadata and page share the call.
--
-- Also trims the embedded review to the columns the page and JSON-LD use
-- (it was reviews(*), i.e. the full review body on every set page render).
--
-- Read-only (STABLE, SECURITY INVOKER): RLS and grants of the underlying
-- tables still apply to the caller. EXECUTE for service_role only -- the set
-- page reads with createServerClient(). No storage: projected DB size impact
-- 0 bytes (catalog entry only).
...
COMMENT ON FUNCTION public.set_page_data(text, text) IS
  'Everything /sets/[slug] renders, in one read-only call (Fix A, 2026-09-26). See migration 20260926030000.';
