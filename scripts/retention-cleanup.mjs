...
async function archiveContentQualityIssues() {
  ...
  // `LIKE content_quality_issues INCLUDING ALL`).
  ...
  if (insErr) throw new Error(`content_quality_issues_archive insert failed: ${insErr.message}`);
  console.log(`[content_quality_issues] Archived ${archiveRows.length} row(s).`);
  ...
  // via a single query rather than deleting one-by-one and catching FK
  // errors (that would be silent-partial-success dressed up as retention).
  const { data: referenced, error: refErr } = await supabase
    .from('content_fix_log')
    .select('issue_id')
    .in('issue_id', ids);
  if (refErr) throw new Error(`content_fix_log reference check failed: ${refErr.message}`);
  const referencedIds = new Set((referenced ?? []).map(r => r.issue_id));
  ...
  const { error: delErr } = await supabase
    ...
    .delete()
    .in('id', deletableIds);
  if (delErr) throw new Error(`content_quality_issues delete failed: ${delErr.message}`);
  console.log(`[content_quality_issues] Deleted ${deletableIds.length} row(s) from the live table.`);
}
