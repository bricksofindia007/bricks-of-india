...
**Tables:** `news_articles` (includes Opinion, `category='Opinion'`), `guides`, `reviews`. `blog_posts` is dormant since the Nav & Content Overhaul (2026-08-09) — Blog/Opinion retired as standalone sections (`/blog`, `/opinion` 308-redirect before ever reaching a page that queries it). Its 25 rows were confirmed byte-for-byte-equivalent (±cosmetic whitespace) duplicates already copied into `guides`/`news_articles`, backed up to `docs/archive/blog_posts_dormant_backup_2026-09-21.json`, and deleted 2026-09-21 — the table is now permanently empty by design. Several audit/hygiene scripts still reference it defensively (harmless on an empty table); see issue filed 2026-09-21 for the low-priority cleanup of those references.
