...
**Publishing policy (superseded 2026-06-28, re-confirmed 2026-09-22 during a write-job audit — issue #170):** `generate-approved-drafts.ts` generates a draft's body and, on full pass of all lint/factuality/voice gates, auto-publishes it directly — no separate publish cron. A draft that fails any gate is **rejected and deleted outright** (`generate-approved-drafts.ts`'s own dated comment: "a row sitting in failed_lint/draft forever provides no value and was the dominant contributor to the unbounded backlog growth"), not parked for later republishing. The older `publish-drafts.yml` cron + `status='failed_lint'` parking/manual-approve-then-republish model this section used to describe is retired (workflow deleted, `scripts/publish-drafts.mjs` kept only as a deprecated reference) — it queried a status value that stopped receiving real input the same day this policy took effect, and ran 3×/day reporting `success` while publishing nothing for roughly 3 months before being found. `/admin/pending`'s reject action sets `status='rejected'`, not a delete — nothing currently reads that status, so it's an inert dead end, not literal deletion.
