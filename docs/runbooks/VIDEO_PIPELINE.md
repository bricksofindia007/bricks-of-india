...
**Supervised approval continues indefinitely as the default — minimum 2-week trial from 2026-07-06.** Daily generation (`video-generate-daily.yml`, 6:00 AM IST) runs in the cloud automatically and leaves each result in `status='pending_approval'`. The operator reviews via email/chat and explicitly approves each one; the Publish Poller (`video-publish-poller.yml`, active, every 15 minutes) then publishes — there is no fixed daily post time, it publishes shortly after approval whenever that happens, **subject to a one-publish-per-IST-calendar-day cap** (`engine.already_published_today_ist()`, added 2026-07-07 after two videos published the same day on 2026-07-06 — root cause: no day-cap existed anywhere before that fix). If something already posted today, the poller holds all `approved` rows untouched until tomorrow; if multiple rows are approved at once with nothing posted yet today, only the oldest-queued one publishes this run and the rest roll to subsequent days.
