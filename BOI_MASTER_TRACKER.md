...
| PARSER-01 | rss-parser malformed feeds — New Elementary (Tier 1), Brickset (Tier 2) | 🟡 Partial | Brickset: ✅ Fixed 2026-05-09 — wrong URL (`/article/rss` returned HTML); correct URL `/feed` returns 40 items cleanly. New Elementary: still open — feed has 3 cascading XML violations (bare attrs, unescaped &, mismatched tags). sanitizeXml() helper added to fetch-rss.js handles first two; third requires full parser swap. Jay's Brick Blog added as new Tier 1 source (confirmed working). |
...
| RADAR-04-FULLTEXT | RADAR-04 drafts from 500-char excerpt only — full article body not fetched | 🟡 P3 deferred | RADAR-04 reads `source_excerpt` (500 chars) stored by RADAR-03. For Tier 1 editorial sources (Brothers Brick, BrickNerd), the full article body is available at source_url and would give Gemini significantly more signal. Improvement: fetch full article via cheerio before Gemini call for source_tier=1 signals. Not blocking — 500-char excerpts are sufficient for v1 drafts. Revisit after first batch of real RADAR-04 output is reviewed. |
