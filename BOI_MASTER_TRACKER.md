...
- **Status:** Fix confirmed working, but does not close the backlog problem. Capacity sizing run live against Supabase (`generator_runs`, `pending_drafts`) 2026-06-28T17:54Z.
  ...
  - **But net throughput is still far below RUN_LIMIT=20:** post-fix scheduled runs succeeded **5/20 (06-27)** and **7/20 (06-28)** — the rest split between `routed_to_review` (Gate 7 hard-rule fails → `status='draft'`, awaiting manual approval) and `failed_lint`. This is gate behavior working as designed, not a bug — but it means real net output is ~6 drafts/day, not 20.
  ...
  - **Net result: backlog is still growing**, ~12/day net, even after the BOM fix. Current state: 536 `approved` rows, 532 with `draft_body IS NULL` (true backlog), oldest dating to **2026-05-09** (50 days old, FIFO).
  - **What this means for RUN_LIMIT:** raising RUN_LIMIT alone won't close the gap on its own — most of the shortfall is downstream of Gemini/Cerebras calls (lint/Gate-7 review routing), not API throughput. Raising RUN_LIMIT to 30+ would only help proportionally to the ~30-35% real pass-through rate seen so far (i.e. ~10/day net instead of ~6/day net) — still short of the 18/day inflow.
  - **Open decision for Abhinav:** (a) raise RUN_LIMIT as a partial mitigation, (b) investigate why Gate 7 routes ~35-45% of Gemini attempts to manual review — is the hard-rule fail rate higher on real traffic than the 49-article calibration corpus suggested, (c) accept the backlog and clear it via batch manual-approval sessions on the `draft` queue, or (d) some combination. Two post-fix data points is not enough to project a stable rate — recheck after 5+ more scheduled runs.
- **Owner:** Terminal (re-measure after 5 more runs) → Abhinav (RUN_LIMIT / Gate-7 tuning decision).
- **Target window:** Before backlog depth reads as a stale-pipeline signal for Fan CoLab evidence.
- **Dependencies:** Gemini 2.5 Flash-Lite free-tier 10 RPM ceiling; Cerebras failover capacity; Gate 7 hard-rule fail rate (CRITICAL-4).
