...
- **Resolution path — no code change needed:** `/admin/pending` (default view, `status=draft` filter) already shows all 200 rows and already has a working bulk action: **"Approve all 200"** (calls `approveAll` — filters only on `status='draft'` + `iteration_label IS NULL`, both satisfied by all 200 rows; confirmed via live query). One click moves all 200 into the `approved` queue, where they then flow through the normal generation pipeline (and benefit from tonight's HIGH-52 fixes once those patches land).
- **Status:** Open — Abhinav action required (click "Approve all" or review individually; some signals are 27 days old and may now be stale/superseded — reviewing before bulk-approving may be worth the time given CRITICAL-2's dedup history).
- **Owner:** A (the approve/reject decision is editorial, not technical).
- **Target window:** Before this reads as a 4-week-stale pipeline signal for Fan CoLab evidence — same concern HIGH-52 already raises for the generation backlog.
...
- **Status:** not started
- **Owner:** C
- **Target window:** unscheduled — low urgency while the queue stays near-empty
- **Dependencies:** None
